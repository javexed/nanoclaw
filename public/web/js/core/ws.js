...
import { state, setLastSeenMessageId } from './state.js';
        ...
        switch (msg.type) {
            case 'system': {
                if (msg.message && !state.myIdentity) {
                    const m = msg.message.match(/^Connected as\s+(.+)$/);
                    if (m)
                        state.myIdentity = m[1].trim();
                }
            ...
            case 'history': {
                ...
                for (const row of carried) {
                    if (row.id && messages.some((m) => m.id === row.id))
                        continue; // echo raced us — server copy already rendered
            ...
            case 'message': {
                ...
                if (wasNearBottom && !state.userScrolledAway)
                    followScroll();
                else if (msg.sender !== state.myIdentity)
                    incrementMissed();
                break;
            }
            case 'typing': {
...
export async function catchUpSince() {
    if (!state.currentRoom || !state.lastSeenMessageId)
        return;
    const room = state.currentRoom; // pin against a mid-fetch room switch
    try {
        const res = await authFetch(`/api/history/${encodeURIComponent(room)}?after=${encodeURIComponent(state.lastSeenMessageId)}`);
        if (!res.ok)
            return;
        if (state.currentRoom !== room)
            return; // switched rooms — don't cross-contaminate
