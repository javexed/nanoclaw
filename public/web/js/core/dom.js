...
export function lucide(name, cls = '') {
...
export function lucideEl(name, cls = '') {
...
/** addEventListener for an async listener. The DOM expects a handler that
 * returns void; handing it one that returns a promise means the event system
 * can neither await nor observe the rejection, which is what
 * no-misused-promises objects to. So the promise is discarded here instead.
 * Handlers surface their own expected failures (a toast, an inline error) —
 * anything still rejecting at this boundary is a bug, so it goes to the
 * console rather than disappearing. */
export function onAsync(el, type, handler) {
    el.addEventListener(type, (e) => {
        void handler(e).catch((err) => {
            console.error(`Unhandled rejection in "${type}" handler`, err);
        });
    });
}
