...
//   - The inbox: approvals whose agent has no web room reach the owner as
...
export function buildApprovalCard(msg) {
    let data = {};
    try {
    ...
    if (resolved) {
        ...
        card.appendChild(optionButtons(data.questionId, data.options ?? [], () => {
...
export function handleApprovalResolved(msg) {
    if (!msg.approvalId)
        return;
...
// ── Inbox surface (agents without a web room) ────────────────────────────
const inboxToasts = new Map();
function dismissInboxToast(questionId) {
    inboxToasts.get(questionId)?.remove();
    inboxToasts.delete(questionId);
}
function showInboxApproval(payload) {
    const questionId = payload.questionId;
    if (!questionId || inboxToasts.has(questionId))
        return;
