...
## Upstream files touched — 13 files, +190 −0
...
|---|---|---|
| `container/agent-runner/src/poll-loop.ts` | +76 | provider-message observers at batch/turn boundaries; module commands (defer/rewrite); turn context; turn retry |
...
| `src/container-runner.ts` | +12 | pre-spawn hooks; gateway key re-pointed by a trailing spread, on the create path and both adoption paths (a surviving session keeps the identity it was created with); container-exit observer; module env into the contributed lane |
...
| `src/delivery.ts` | +3 | session delivery observers after both polls; producing session passed to deliver() |
