...
## Upstream files touched — 13 files, +185 −0
...
|---|---|---|
| `container/agent-runner/src/poll-loop.ts` | +72 | provider-message observers at batch/turn boundaries; module commands (defer/rewrite); turn context; turn retry |
...
| `container/agent-runner/src/providers/types.ts` | +10 | `ProviderOptions.moduleInput` — module-owned per-query markers; a field rather than a side table so a wrapper copying the input cannot silently drop them |
