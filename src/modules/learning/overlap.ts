 ...
 *      candidates are judged by a local model over an Anthropic-format
 *      /v1/messages endpoint — Ollama serves one, so the default URL is the
 *      local Ollama root. No cloud spend, a few seconds, and it catches
 *      overlap the token heuristic can't see.
 */
import fs from 'fs';
...
import { listSkillDrafts, readSkillDraftBody, type SkillDraft } from './db.js';
...
export async function gatherOverlapCandidates(
  ...
  for (const poolRoot of [
    ...
    path.join(process.cwd(), 'data', 'user-skills'),
  ]) {
    try {
...
/**
 * Optional LLM judge over the heuristic shortlist, over an Anthropic-format
 * /v1/messages endpoint (Ollama serves one) — configured via .env:
 *   NANOCLAW_OVERLAP_MODEL=llama3.2   (unset = heuristic only)
 *   NANOCLAW_OVERLAP_URL=http://127.0.0.1:11434   (default: local Ollama)
...
async function llmJudge(
  ...
  const url = (env.NANOCLAW_OVERLAP_URL || 'http://127.0.0.1:11434').replace(/\/$/, '');
