 ...
 * identifier is `byok-<userSlug>-<hash>`; the host recovers the owning agent
 * group via the `byok_credentials` table (onecli_agent_id → agent_group_id),
 ...
 */
export function byokAgentIdentifier(agentGroupId: string, userId: string): string {
...
/** True if an OneCLI identifier was minted for a BYOK per-member agent. */
export function isByokAgentIdentifier(identifier: string | null | undefined): boolean {
