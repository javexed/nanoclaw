 ...
 * installed module reacts to the session going away (e.g. reconcile UI state it
 * derived from the running container). Fire-and-forget and isolated: a throwing
 * observer can never affect container lifecycle. Core ships none.
 *
 * Since the driver seam landed upstream, the ONLY place this can fire from is
 * `armSessionLifecycle`'s `onTerminal` — the old `container.on('error')` and
 * stderr handlers no longer exist here, the driver owns the process. That is a
 * better fit than what it replaced: onTerminal fires at most once and only on
 * ends the host did not request, so an observer no longer has to work out
 * whether a stop was intentional.
 */
...
async function spawnContainer(session: Session): Promise<void> {
  ...
  const gateway = await getGatewayProvider().contribute({
    // agentIdentifier, NOT agentGroup.id: the gateway uses key.agentGroupId as
    // the OneCLI agent (ensureAgent + applyContainerConfig({ agent })), so a
    // module can re-point a per-member session at THAT member's agent and get
    // their key injected. Passing the group id here would silently run the
    // container on the workspace identity — the exact wrong-identity failure
    // registerAgentIdentityResolver exists to prevent.
    //
    // The SessionSpec's own key (in composeSessionSpec) deliberately keeps
    // agentGroup.id: that one is the driver's session identity for labels and
    // admission joins, not a credential.
    key: { installSlug: INSTALL_SLUG, agentGroupId: agentIdentifier, sessionId: session.id },
  ...
  try {
    await armSessionLifecycle({
      handle,
      onTerminal: (failure) => {
        // Seam hook kept; upstream added `runtime` as the second parameter.
...
export function composeSessionSpec(input: ComposeSessionSpecInput): SessionSpec {
  ...
  const contributedEnv: Record<string, string> = {
    ...(contribution.env ?? {}),
    ...(gateway.env ?? {}),
    // Module-contributed env goes LAST so it wins a key collision — the same
    // precedence the old raw-argv path gave it, where the last `-e` won. It
    // rides the CONTRIBUTED lane, not the composed one: that lane is exempt
    // from the credential-NAME check (a module may legitimately name a
    // credential-ish var) while still refusing credential-shaped VALUES.
    ...(extraEnv ?? {}),
  };
