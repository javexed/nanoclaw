...
let planForTest: (() => import('./seam/routing-hooks.js').InboundDeliveryPlan | null) | null = null;
let keyOverrideForTest: ((agentGroupId: string) => import('./seam/session-hooks.js').SessionKeyOverride | null) | null =
  null;
let hooksRegistered = false;
async function registerTestHooks() {
  if (hooksRegistered) return;
  hooksRegistered = true;
  const router = await import('./seam/routing-hooks.js');
  const sm = await import('./seam/session-hooks.js');
