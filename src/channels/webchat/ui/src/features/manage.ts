...
function showTab(tab: string): void {
  ...
  else if (tab === 'models') void renderModels();
  else void renderOllama();
}
...
function buildModelCreate(knownAnthropic: string[]): HTMLElement {
  ...
  discover.addEventListener('click', async () => {
    ...
    try {
      const { models } = (await apiJson('/api/models/discover', { method: 'POST', body: { endpoint: ep } })) as {
        models: string[];
      };
...
async function renderOllama(): Promise<void> {
