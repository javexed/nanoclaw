...
async function renderModels(): Promise<void> {
  ...
  try {
    const data = (await apiJson('/api/models')) as {
      models: ModelRow[];
    ...
    pane.replaceChildren(
      ...
      ollamaBox,
      msection('Add custom endpoint'),
      buildCustomEndpoint(rosterKeys),
    );
...
function buildCustomEndpoint(rosterKeys: Set<string>): HTMLElement {
  const box = document.createElement('div');
  box.className = 'mrow mcreate';
...
async function renderOllamaInto(pane: HTMLElement, rosterKeys: Set<string>): Promise<void> {
  pane.replaceChildren();
  try {
    const { hosts } = (await apiJson('/api/ollama/hosts')) as { hosts: string[] };
    ...
    const refreshModels = async (): Promise<void> => {
      try {
        const { models } = (await apiJson(`/api/ollama/models?host=${encodeURIComponent(hostSel.value)}`)) as {
          models: Array<{ name: string; size: number; loaded: boolean }>;
        };
    ...
    const refreshedFor = new Set<string>(); // pulls whose success already triggered one refresh
    const refreshPulls = async (): Promise<void> => {
      try {
        const { pulls } = (await apiJson('/api/ollama/pulls')) as { pulls: PullRow[] };
        pullsBox.replaceChildren(
          ...pulls.map((p) => {
            const row = document.createElement('div');
            row.className = 'mpull';
        ...
        if (fresh.length > 0) {
          for (const p of fresh) refreshedFor.add(`${p.host}|${p.model}`);
          void refreshModels();
        }
      } catch {
        /* transient */
  ...
  try {
    ...
    btn.addEventListener('click', async () => {
      ...
      for (let i = 0; i < 200; i++) {
        ...
        try {
          const st = (await apiJson('/api/ollama/local')) as {
            reachable: boolean;
            running?: boolean;
            lines?: string[];
