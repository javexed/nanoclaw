...
const turns = new Map<string, Turn>(); // agentName → open turn

function container(): HTMLElement {
  return $('#thinking')!;
}

function openTurn(agentName: string): Turn {
...
function closeTurn(agentName: string): void {
...
export function closeTurnFor(agentName: string): void {
  closeTurn(agentName);
}

export function clearAllTurns(): void {
  for (const name of [...turns.keys()]) closeTurn(name);
}
...
export function handleStatusEvent(msg: {
  ...
  switch (msg.event) {
    ...
    case 'progress': {
      ...
      if (msg.text) turn.activity.textContent = truncate(msg.text, 100);
      break;
    }
    case 'reasoning': {
      const turn = openTurn(agentName);
    ...
    case 'stalled':
      closeTurn(agentName);
      if (msg.text) appendSystem(`⚠ ${agentName || 'Agent'}: ${msg.text}`);
      break;
  }
}
