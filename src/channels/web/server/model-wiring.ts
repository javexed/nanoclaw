...
export async function reloadAgentModelEnv(agentGroupId: string, reason: string): Promise<void> {
  try {
    ...
    log.warn('Web: settings.json write after model change failed', { agentGroupId, reason, err });
  }
  try {
    ...
    log.warn('Web: provider sync after model change failed', { agentGroupId, reason, err });
  }
  try {
    const restarted = await restartAgentGroupContainers(agentGroupId, reason);
    if (restarted > 0) {
      log.info('Web: restarted containers after model change', { agentGroupId, reason, restarted });
    }
  } catch (err) {
    log.warn('Web: container restart after model change failed', { agentGroupId, reason, err });
  }
}
...
export async function refreshUnassignedGroupsForDefaultModel(reason: string): Promise<void> {
  for (const g of await getAllAgentGroups()) {
    ...
    try {
      ...
      log.warn('Web: settings.json write for default-model change failed', { agentGroupId: g.id, reason, err });
    }
    try {
