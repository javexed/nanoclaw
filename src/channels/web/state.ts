/**
 * Web in-memory client registry + broadcast.
 *
...
export interface WSClient {
  ...
  /** v2-namespaced user id (`web:owner` / `web:local-owner`). */
  userId: string;
  room_id?: string;
  isAlive: boolean;
}
...
export async function broadcast(roomId: string, msg: object, excludeId?: string): Promise<void> {
  ...
  if (isMessage && inRoom === 0 && elsewhere > 0) {
    log.warn('Web: message broadcast with no client in the room', { roomId, clientsElsewhere: elsewhere });
  }
}
...
export function pushApprovalToUser(userId: string, askQuestionPayload: Record<string, unknown>): void {
  ...
  if (sent === 0) {
    log.info('Web approval queued for offline user', { userId });
  }
}
