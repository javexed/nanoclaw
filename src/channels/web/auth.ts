/**
 * Web HTTP/WS authentication — single-user build.
 *
 * Two methods, tried in order:
 *   - bearer token   WEB_TOKEN set; matched constant-time. The PWA passes
 ...
 *
 * Identities: `web:owner` (bearer), `web:local-owner` (localhost).
...
// Bind host (see channels/web/index.ts — default 127.0.0.1). A loopback
...
const WEB_HOST = (process.env.WEB_HOST || '127.0.0.1').trim();
...
export async function authenticateRequest(req: IncomingMessage): Promise<AuthResult | AuthFailure> {
  ...
  if (result.ok) {
    ...
    if (!loggedSessions.has(key)) {
      ...
      log.info('Web auth session', { userId: result.userId, source: result.source, ip: remoteIp });
    }
  } else {
    const last = loggedDenials.get(remoteIp) ?? 0;
    if (Date.now() - last > 60_000) {
      loggedDenials.set(remoteIp, Date.now());
      log.warn('Web auth denied', { ip: remoteIp });
    }
  }
  return result;
}
...
async function authenticate(req: IncomingMessage): Promise<AuthResult | AuthFailure> {
  ...
  if ((await bearerActive()) && providedToken && safeEqual(providedToken, WEB_TOKEN)) {
    return finalize({ source: 'bearer', userId: 'web:owner', displayName: 'operator' });
  }
  ...
  if (isLocalhost(remoteIp) && !(await hasExplicitAuth())) {
    ...
    return finalize({ source: 'localhost', userId: 'web:local-owner', displayName: localUser });
  }
 ...
 * not retired by the owner (web_settings.bearer_token_disabled).
 */
async function bearerActive(): Promise<boolean> {
  return Boolean(WEB_TOKEN) && !(await getBearerTokenDisabled());
}
...
export function assertBearerTokenStrength(): void {
  if (WEB_TOKEN && WEB_TOKEN.length < MIN_BEARER_TOKEN_LENGTH) {
    throw new Error(
      `Web refusing to start: WEB_TOKEN is ${WEB_TOKEN.length} chars, ` +
 ...
 * squeeze a second INSERT through. Both web identities (bearer's
 * `web:owner` and loopback's `web:local-owner`) funnel through here;
...
async function ensureOwnerRoleOnFirstLogin(userId: string): Promise<void> {
  ...
  try {
    ...
    if (result.changes > 0) {
      log.info('Web: granted owner role to first authenticated user', { userId });
    }
  } catch (err) {
    log.warn('Web: failed to grant initial owner role', { userId, err });
  }
}
...
async function finalize(args: {
  ...
  if (await hasTable(getDb(), 'users')) {
    try {
      ...
      log.warn('Web: upsertUser failed during auth finalize', { userId: args.userId, err });
    }
  }
