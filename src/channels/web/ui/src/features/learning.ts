...
function overlapsFromError(err: unknown): Overlap[] | null {
  const e = err as { body?: { overlaps?: unknown }; data?: { overlaps?: unknown }; message?: string };
  for (const cand of [e?.body?.overlaps, e?.data?.overlaps]) {
    if (Array.isArray(cand)) return cand as Overlap[];
  }
  try {
    const m = /\{[\s\S]*\}/.exec(String(e?.message ?? ''));
    if (m) {
      const j = JSON.parse(m[0]) as { overlaps?: unknown };
      if (Array.isArray(j.overlaps)) return j.overlaps as Overlap[];
    }
  } catch {
    /* not JSON */
  }
...
function flip(card: HTMLElement, outcome: 'kept' | 'discarded', resolvedBy?: string): void {
  card.classList.add('appr-resolved');
  card.querySelector('.appr-actions')?.remove();
  card.querySelector('.draft-body')?.remove();
  let note = card.querySelector<HTMLElement>('.appr-note');
  if (!note) {
...
async function keep(draftId: string, force: boolean): Promise<void> {
...
function actionRow(card: HTMLElement, data: DraftPayload): HTMLElement {
  const row = document.createElement('div');
  row.className = 'appr-actions';
  const draftId = data.draftId ?? '';
  const setBusy = (busy: boolean) =>
    row.querySelectorAll('button').forEach((b) => ((b as HTMLButtonElement).disabled = busy));
  ...
  onAsync(view, 'click', async (e) => {
    ...
    try {
      const d = (await apiJson(`/api/skill-drafts/${encodeURIComponent(draftId)}`)) as { body?: string };
...
export function buildSkillDraftCard(msg: { content?: string; message_type?: string }): HTMLElement {
  let data: DraftPayload = {};
  try {
    data = JSON.parse(msg.content ?? '{}') as DraftPayload;
  } catch {
    /* render as unparsed */
  }
...
export function handleSkillDraftResolved(msg: { draftId?: string; outcome?: string; resolvedBy?: string }): void {
  if (!msg.draftId) return;
  const card = document.querySelector<HTMLElement>(`.appr-card[data-draft-id="${CSS.escape(msg.draftId)}"]`);
  if (!card) return;
