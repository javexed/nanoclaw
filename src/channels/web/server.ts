/**
 * Web HTTP server — single-user build.
 *
...
export async function startWebServer(hooks: WebServerHooks): Promise<WebServer> {
  const host = process.env.WEB_HOST || DEFAULT_HOST;
  ...
  const publicDir = path.resolve(process.env.WEB_PUBLIC_DIR || 'public/web');
  ...
  if (requiresExplicitAuth(host) && !(await hasExplicitAuth())) {
    throw new Error(
      `Web refusing to bind to ${host}:${port}: no auth method configured. ` +
        'Set WEB_TOKEN, or bind to 127.0.0.1 instead.',
    );
  }
  ...
  if ((tlsCert && !tlsKey) || (!tlsCert && tlsKey)) {
    log.warn('Web: both WEB_TLS_CERT and WEB_TLS_KEY must be set for HTTPS — falling back to HTTP');
  }

  const requestHandler = (req: IncomingMessage, res: ServerResponse): void => {
    void handleHttp(req, res, hooks, publicDir).catch((err) => {
      log.error('Web HTTP handler threw', { err });
      if (!res.headersSent) {
        // Web is single-tenant + auth-gated; surface err.message so the
  ...
  await new Promise<void>((resolve, reject) => {
    httpServer.once('error', (err: NodeJS.ErrnoException) => {
      ...
      if (err.code === 'EADDRINUSE') {
        log.error(`Web: port ${port} already in use — another nanoclaw host is likely running for this checkout.`, {
          host,
          port,
        });
      }
...
export async function stopWebServer(server: WebServer): Promise<void> {
  // close() waits for every open socket — and a web server ALWAYS has
...
async function handleHttp(
  ...
  // Uploaded/outbound files (auth-gated — private attachments).
  {
    ...
    if (up && (method === 'POST' || method === 'PUT')) {
      if (req.headers['x-web-csrf'] !== '1') return json(res, 403, { error: 'Missing X-Web-CSRF header' });
  ...
  for (const r of API_ROUTES) {
    ...
    for (const g of r.guards ?? []) {
      if (g === 'csrf' && req.headers['x-web-csrf'] !== '1')
        return json(res, 403, { error: 'Missing X-Web-CSRF header' });
    }
...
async function rRoomNamePut({ req, res }: RouteCtx, m: RegExpMatchArray): Promise<void> {
  ...
  const name = sanitizeRoomName(body.name);
  if (!name) return json(res, 400, { error: 'Room name required (1-80 printable chars)' });
  await updateWebRoomName(roomId, name);
  await broadcastRooms();
...
async function rHistoryGet({ res, url }: RouteCtx, m: RegExpMatchArray): Promise<void> {
  ...
  const messages = before
    ? await getWebMessagesBeforeId(roomId, before, limit)
    : after
      ? await getWebMessagesAfterId(roomId, after, limit)
      : await getWebMessages(roomId, limit);
  return json(res, 200, {
    room_id: roomId,
...
async function rApprovalsPendingGet({ res, userId }: RouteCtx): Promise<void> {
  const rows = await getWebPendingApprovalsForUser(userId);
  return json(res, 200, {
...
const API_ROUTES: ApiRoute[] = [
  ...
  { method: 'GET', path: '/api/web/onboarding', h: rOnboardingGet },
  { method: 'PUT', path: '/api/web/onboarding', guards: ['csrf'], h: rOnboardingPut },
  { method: 'POST', path: '/api/web/auth/bearer/generate', guards: ['csrf'], h: rBearerGeneratePost },
  { method: 'POST', path: '/api/web/claude-auth/start', guards: ['csrf'], h: rClaudeAuthStartPost },
  { method: 'POST', path: '/api/web/claude-auth/code', guards: ['csrf'], h: rClaudeAuthCodePost },
  { method: 'POST', path: '/api/web/claude-auth/cancel', guards: ['csrf'], h: rClaudeAuthCancelPost },
  { method: 'POST', path: '/api/web/restart', guards: ['csrf'], h: rRestartPost },
  { method: 'GET', path: '/api/web/opencode', h: rOpencodeGet },
  { method: 'POST', path: '/api/web/opencode/install', guards: ['csrf'], h: rOpencodeInstallPost },
  { method: 'GET', path: '/api/web/tailscale-https', h: rTailscaleHttpsGet },
  { method: 'POST', path: '/api/web/tailscale-https', guards: ['csrf'], h: rTailscaleHttpsPost },
  { method: 'GET', path: '/api/web/tailscale/install', h: rTailscaleInstallGet },
  { method: 'POST', path: '/api/web/tailscale/install', guards: ['csrf'], h: rTailscaleInstallPost },
