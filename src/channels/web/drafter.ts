...
import { getDefaultModelId, getWebModel, type WebModel } from './db.js';
...
export function ensureDrafterIdentity(): Promise<void> {
  ...
  bootstrapPromise = onecli
    ...
    .then(() => {
      log.info('Web drafter identity registered with OneCLI', { identifier: DRAFTER_AGENT_ID });
    })
    .catch((err) => {
...
export async function anthropicMessagesViaOneCLI(call: AnthropicMessagesCall): Promise<string> {
  ...
  if (res.status === 401) {
    ...
    throw new DraftError(
      'OneCLI rejected the call (401). The web-drafter agent must be in `all` secret mode — nothing assigns it a secret, so `selective` leaves it with none. Check with `onecli agents list --max 500` (the list silently caps at 20) and fix with: onecli agents set-secret-mode --id <internal-id> --mode all',
      503,
    );
  }
  if (!res.ok) {
...
async function runDraft(prompt: string): Promise<DraftedAgent> {
  ...
  const defaultModel = defaultId ? await getWebModel(defaultId) : undefined;
...
async function runDraftViaModel(prompt: string, model: WebModel): Promise<DraftedAgent> {
  let res: Response;
  try {
    ...
    log.warn('Web drafter: model endpoint fetch threw', { err, model: model.model_id });
    throw new DraftError('Drafter call to the local model failed (see server logs)', 503);
  }
  if (!res.ok) {
    const detail = await res.text().catch(() => '');
    log.warn('Web drafter: model endpoint non-OK', { status: res.status, detail: detail.slice(0, 300) });
