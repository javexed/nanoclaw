/**
 * Web DB helpers — typed CRUD over the tables created by migration.ts.
 *
 * Does NOT replace inbound.db / outbound.db — the adapter mirrors agent
 * traffic into web_messages so the PWA has a unified history view, but
 ...
 * channel_type='web'`. The room id surfaces as
...
interface WebMessageRow {
  ...
  sender_type: string;
  content: string;
  message_type: WebMessage['message_type'];
  file_meta: string | null;
  created_at: number;
}

function rowToMessage(row: WebMessageRow): WebMessage {
  return {
    ...row,
...
function rowToRoom(row: { platform_id: string; name: string | null; created_at: string }): WebRoom {
  return {
    id: row.platform_id,
...
export async function createWebRoom(name: string, id?: string): Promise<WebRoom> {
  ...
  const existing = await getMessagingGroupByPlatform('web', platformId);
...
export async function getWebRoom(id: string): Promise<WebRoom | undefined> {
  const mg = await getMessagingGroupByPlatform('web', id);
  return mg ? rowToRoom(mg) : undefined;
}

export async function getAllWebRooms(): Promise<WebRoom[]> {
...
export async function updateWebRoomName(id: string, name: string): Promise<void> {
  await getDb().run(`UPDATE messaging_groups SET name = ? WHERE channel_type='web' AND platform_id = ?`, name, id);
}

/**
 * Delete a web room and everything that hangs off it: messages, the
 ...
 */
export async function deleteWebRoom(id: string): Promise<void> {
  const mg = await getMessagingGroupByPlatform('web', id);
  if (!mg) return;
  const db = getDb();
...
export async function getRoomLastActivity(): Promise<Map<string, number>> {
  const rows = (await getDb().all(`SELECT room_id, MAX(created_at) AS last_at FROM web_messages GROUP BY room_id`)) as {
    room_id: string;
    last_at: number;
  }[];
...
// The web adapter exposes openDM() returning `approvals:<handle>` so
...
/** `web:owner` → `approvals:owner`. Null for non-web user_ids. */
export function approvalInboxForUser(userId: string): string | null {
  if (!userId.startsWith('web:')) return null;
  return `${APPROVAL_INBOX_PREFIX}${userId.slice('web:'.length)}`;
}
...
export function userForApprovalInbox(platformId: string): string | null {
  ...
  return `web:${platformId.slice(APPROVAL_INBOX_PREFIX.length)}`;
}
...
/**
 * Record an approval delivered to a web surface (an inbox or a room
 ...
 * side-table is where web remembers which surfaces rendered which card.
 */
export async function recordWebApproval(approvalId: string, platformId: string): Promise<void> {
  await getDb().run(
    `INSERT OR IGNORE INTO web_approvals_index (approval_id, platform_id, recorded_at)
       VALUES (?, ?, ?)`,
    approvalId,
    platformId,
    Date.now(),
...
export async function isWebApprovalIndexedFor(approvalId: string, platformId: string): Promise<boolean> {
  const row = (await getDb().get(
    `SELECT 1 FROM web_approvals_index WHERE approval_id = ? AND platform_id = ? LIMIT 1`,
    approvalId,
    platformId,
...
export async function getWebApprovalInboxes(approvalId: string): Promise<string[]> {
  const rows = (await getDb().all(
    `SELECT platform_id FROM web_approvals_index WHERE approval_id = ? ORDER BY recorded_at`,
    approvalId,
...
export async function deleteWebApprovalIndex(approvalId: string): Promise<void> {
  await getDb().run(`DELETE FROM web_approvals_index WHERE approval_id = ?`, approvalId);
}

/** Pending approvals destined for this user's inbox (joined against the index). */
export async function getWebPendingApprovalsForUser(userId: string): Promise<PendingApprovalRow[]> {
  ...
  return (await getDb().all(
    `SELECT pa.approval_id, pa.action, pa.title, pa.options_json, pa.payload, pa.created_at
         ...
         JOIN web_approvals_index wai ON wai.approval_id = pa.approval_id
...
export async function storeWebMessage(
  roomId: string,
  sender: string,
  senderType: string,
  content: string,
  ...
  await getDb().run(
    `INSERT INTO web_messages (id, room_id, sender, sender_type, content, message_type, file_meta, created_at)
...
export async function storeWebFileMessage(
  roomId: string,
  sender: string,
  senderType: string,
  caption: string,
 ...
 * resolution.
 */
export async function storeWebApprovalCard(
  roomId: string,
  sender: string,
  payload: {
    questionId: string;
...
export async function markRoomApprovalResolved(approvalId: string, resolvedBy: string): Promise<void> {
  ...
  const row = (await getDb().get(`SELECT content FROM web_messages WHERE id = ?`, id)) as
    | { content: string }
    | undefined;
  if (!row) return;
  ...
  await getDb().run(
    `UPDATE web_messages SET message_type = 'approval_resolved', content = ? WHERE id = ?`,
    JSON.stringify(payload),
    id,
  );
}

export async function getWebMessages(roomId: string, limit = 200): Promise<WebMessage[]> {
  const rows = (await getDb().all(
    `SELECT * FROM web_messages WHERE room_id = ? ORDER BY created_at DESC LIMIT ?`,
    roomId,
    limit,
  )) as WebMessageRow[];
 ...
 */
export async function deleteWebMessage(messageId: string, requesterIdentity: string, roomId: string): Promise<boolean> {
  const result = await getDb().run(
    `DELETE FROM web_messages WHERE id = ? AND sender = ? AND room_id = ?`,
    messageId,
    requesterIdentity,
    roomId,
  );
...
export async function getWebMessagesAfterId(roomId: string, afterId: string, limit = 500): Promise<WebMessage[]> {
  const anchor = (await getDb().get(`SELECT created_at, id FROM web_messages WHERE id = ?`, afterId)) as
 ...
 */
export async function getWebMessagesBeforeId(roomId: string, beforeId: string, limit = 50): Promise<WebMessage[]> {
  const anchor = (await getDb().get(`SELECT created_at, id FROM web_messages WHERE id = ?`, beforeId)) as
    | { created_at: number; id: string }
    | undefined;
  if (!anchor) return [];
  const rows = (await getDb().all(
...
// ── Install settings (web_settings singleton) ───────────────────────────
...
function settingsGetter<T>(column: string, decode: (value: unknown) => T): () => Promise<T> {
  return async () => {
    try {
      const row = (await getDb().get(`SELECT ${column} FROM web_settings WHERE id = 1`)) as
...
function settingsSetter<V>(column: string, encode: (value: V) => string | number | null): (value: V) => Promise<void> {
  return async (value) => {
    await getDb().run(`UPDATE web_settings SET ${column} = ?, updated_at = ? WHERE id = 1`, encode(value), Date.now());
  };
}
...
/** Bearer-token opt-out: true = WEB_TOKEN is ignored by auth.ts. */
...
export type WebModelKind = 'anthropic' | 'ollama' | 'openai-compatible';
...
export async function listWebModels(): Promise<WebModel[]> {
  return (await getDb().all(`SELECT * FROM web_models ORDER BY name COLLATE NOCASE`)) as WebModel[];
}

export async function getWebModel(id: string): Promise<WebModel | undefined> {
  return (await getDb().get(`SELECT * FROM web_models WHERE id = ?`, id)) as WebModel | undefined;
}

export async function createWebModel(m: WebModel): Promise<void> {
  await getDb().run(
    `INSERT INTO web_models (id, name, kind, endpoint, model_id, credential_ref, created_at)
       VALUES (@id, @name, @kind, @endpoint, @model_id, @credential_ref, @created_at)`,
    m,
  );
}

export async function updateWebModel(
  id: string,
  patch: { name?: string; endpoint?: string | null; model_id?: string; credential_ref?: string | null },
): Promise<void> {
  const existing = await getWebModel(id);
  if (!existing) return;
...
export async function deleteWebModel(id: string): Promise<void> {
  const db = getDb();
  // Cascade in JS — caller is expected to have surfaced the impact list.
  await db.run(`DELETE FROM web_agent_models WHERE model_id = ?`, id);
  await db.run(`DELETE FROM web_models WHERE id = ?`, id);
}

export async function getAgentsAssignedToModel(modelId: string): Promise<string[]> {
  return (
    (await getDb().all(`SELECT agent_group_id FROM web_agent_models WHERE model_id = ?`, modelId)) as {
      agent_group_id: string;
    }[]
  ).map((r) => r.agent_group_id);
}

export async function getAssignedModelForAgent(agentGroupId: string): Promise<WebModel | null> {
  const row = (await getDb().get(`SELECT model_id FROM web_agent_models WHERE agent_group_id = ?`, agentGroupId)) as
    | { model_id: string }
    | undefined;
  if (!row) return null;
  return (await getWebModel(row.model_id)) ?? null;
}
...
export async function getEffectiveModelForAgent(agentGroupId: string): Promise<WebModel | null> {
  const assigned = await getAssignedModelForAgent(agentGroupId);
  if (assigned) return assigned;
  const defaultId = await getDefaultModelId();
  return defaultId ? ((await getWebModel(defaultId)) ?? null) : null;
}

export async function assignModelToAgent(agentGroupId: string, modelId: string): Promise<void> {
  await getDb().run(
    `INSERT INTO web_agent_models (agent_group_id, model_id, assigned_at)
       VALUES (?, ?, ?)
