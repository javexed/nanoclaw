/**
 * Web WebSocket protocol — single-user build.
 *
...
import { deleteWebMessage, getWebMessages, getWebRoom, storeWebMessage } from './db.js';
...
/**
 * Deliver a "stop" signal to live session(s) behind a web room (the GUI
...
export function setupWebSocket(
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    void (async () => {
      ...
      log.warn('Web WS upgrade failed', { err });
      socket.destroy();
    });
  });
  ...
  wss.on('connection', (ws: WebSocket, req: http.IncomingMessage) => {
    ...
    const userId = augmented._authUserId ?? 'web:unknown';
    ...
    ws.on('error', (err) => {
      log.warn('Web WS client error', { clientId, identity: client.identity, err: err.message });
    });
    ...
    const handleFrame = async (raw: Buffer | ArrayBuffer | Buffer[]) => {
      ...
      if (msg.type === 'join') {
        ...
        // its own cause.
        if (!room) {
          log.warn('Web: join refused — room not found', { roomId, userId: client.userId });
        ...
        send({
          type: 'history',
          room_id: room.id,
          messages: (await getWebMessages(room.id, 50)).map((m) => ({
            ...m,
      ...
      if (msg.type === 'message') {
        ...
        if (cid && !claimClientId(cid)) {
          log.warn('Web: dropped duplicate message (client_id already seen)', {
            clientId: cid,
        ...
        const stored = await storeWebMessage(client.room_id, client.identity, client.identity_type, text);
      ...
      if (msg.type === 'delete_message') {
        ...
        const deleted = await deleteWebMessage(messageId, client.identity, client.room_id);
        if (deleted) {
    ...
    ws.on('message', (raw) => {
      frameChain = frameChain
        ...
        .catch((err) => {
          ...
          log.warn('Web: WS frame handler failed', { err: err instanceof Error ? err.message : String(err) });
        });
    });
