/**
 * `tailscale serve` control — the host side of the web "Enable HTTPS over
 * Tailscale" action.
 *
 * `tailscale serve --bg <port>` puts an HTTPS reverse proxy in front of the
 * local web on the node's `*.ts.net` MagicDNS name, using Tailscale's
 ...
 * which auth.ts maps back to the same `web:tailscale:<login>` identity the
