 ...
 *
 * Files land under data/web/uploads/<roomId>/<uuid><.ext>. The agent
...
import { storeWebFileMessage, getWebRoom, type FileMeta } from './db.js';
...
export function uploadsDir(roomId: string): string {
  return path.join(DATA_DIR, 'web', 'uploads', sanitizeId(roomId));
}
...
function inboundForFile(
  ...
  if (fileMeta.size <= INLINE_ATTACHMENT_THRESHOLD) {
    try {
      ...
      log.warn('Web: failed to inline attachment for inbound', {
        localFilePath,
...
export async function handleMultipartUpload(
  ...
  if (!(await getWebRoom(roomId))) {
    log.warn('Web upload rejected: room not found', { roomId });
  ...
  if (!contentType.includes('multipart/form-data')) {
    log.warn('Web upload rejected: bad content-type', { roomId, contentType });
  ...
  busboy.on('finish', () => {
    if (limitHit) {
      log.warn('Web upload rejected: file size limit hit', { roomId });
      return json(res, 413, { error: `File exceeds ${(MAX_UPLOAD_SIZE / 1024 / 1024 / 1024).toFixed(1)}GB limit` });
    }
    if (!fileInfo) {
      log.warn('Web upload rejected: no file part', { roomId, contentType });
    ...
    writeDone
      .then(async () => {
        ...
        const stored = await storeWebFileMessage(roomId, senderIdentity, 'user', caption, fileMeta);
      ...
      .catch((err) => {
        log.warn('Web upload: write stream errored', { roomId, err: err instanceof Error ? err.message : err });
        json(res, 500, { error: 'Upload write failed' });
      });
  });

  busboy.on('error', (err) => {
    log.warn('Web upload failed', { err: err instanceof Error ? err.message : err });
