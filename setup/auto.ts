  ...
  if (!skip.has('webchat') && process.env.NANOCLAW_REEXEC_SG !== '1' && process.stdin.isTTY) {
    ...
    if (enableWebchat === 'yes') {
      ...
      p.log.success(
        brandBody(
          wrapForGutter(
            `Web chat enabled. Once NanoClaw starts (next step), open ${k.bold(`http://127.0.0.1:${webchatPort}/`)} — the first visit walks you through picking a model and creating an agent.`,
            4,
          ),
        ),
      );
