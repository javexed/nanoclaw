#!/usr/bin/env bash
#
# deploy/web-deploy.sh — headless, network-exposed web deploy.
#
# The SINGLE SOURCE OF TRUTH for a non-interactive web install: build the
...
#   --no-tailscale       don't set WEB_TAILSCALE=true (it's on by default)
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) echo "web-deploy: unknown arg: $1" >&2; exit 2 ;;
  esac
done

# Default DIR = the repo root this script lives in (deploy/..), so a plain
# `bash deploy/web-deploy.sh` from a checkout just works.
[ -n "$DIR" ] || DIR="$(cd "$(dirname "$0")/.." && pwd)"
cd "$DIR" || { echo "web-deploy: cannot cd to $DIR" >&2; exit 1; }
say() { echo "→ $*"; }
OS="$(uname -s)"
...
if [ "$INSTALL_DEPS" = 1 ]; then
  [ "$(id -u)" = 0 ] || { echo "web-deploy: --install-deps needs root (apt + Docker install)" >&2; exit 1; }
  command -v apt-get >/dev/null 2>&1 || { echo "web-deploy: --install-deps supports Debian/Ubuntu (apt) only" >&2; exit 1; }
  ...
  # Keeping these version pins current: docs/web/dependency-review.md (reviewed quarterly).
  NODE_MAJOR="$(node -v 2>/dev/null | sed 's/v//; s/\..*//')"
  if [ -z "$NODE_MAJOR" ] || [ "$NODE_MAJOR" -lt 20 ]; then
    echo "" >&2
    echo "web-deploy: apt's Node is '${NODE_MAJOR:-missing}' but NanoClaw needs >= 20 (22 preferred)." >&2
...
# force-set — an add-if-missing left a prior deploy's WEB_HOST=0.0.0.0 in
...
env_set WEB_ENABLED true
env_force WEB_HOST "$HOST"
env_force WEB_PORT "$PORT"
...
[ "$TAILSCALE" = 1 ] && env_set WEB_TAILSCALE true
...
env_set TZ "$TZ_VAL"
say "Wrote .env (web on ${HOST}:${PORT})"
...
echo ""
if [ "$LOCALHOST" = 1 ]; then
  echo "✓ NanoClaw web deployed on 127.0.0.1:${PORT} (localhost only)."
  if grep -q "^WEB_TOKEN=" .env; then
    echo "  A bearer token is set, so log in with it (the loopback auto-owner is off): $(grep '^WEB_TOKEN=' .env | cut -d= -f2-)"
  else
    echo "  Open http://127.0.0.1:${PORT}/ — you're signed in as owner automatically."
  fi
else
  TOKEN_OUT=$(grep '^WEB_TOKEN=' .env | cut -d= -f2- || true)
  echo "✓ NanoClaw web deployed on ${HOST}:${PORT}."
