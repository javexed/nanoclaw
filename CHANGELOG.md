...
- [BREAKING] **Alternative providers moved to the `providers` branch.** Codex and Ollama install via `/add-codex` and `/add-ollama-provider`. Claude remains the default provider baked into trunk. OpenAI-compatible and cloud backends (OpenRouter, DeepSeek, …) run on the default Claude harness via LiteLLM's Anthropic `/v1/messages` surface — install `/add-litellm`.
