 ...
 * (see docs/web/README.md, "Learning").
 *
 ...
 * action; the host stages it as a draft and drops a Keep/Discard card in the
 * agent's room. Nothing runs in any agent until a human keeps it.
 *
...
export const draftSkill: McpToolDefinition = {
  ...
  async handler(args) {
    ...
    return ok(
      kind === 'patch'
        ? `Proposed revision to "${target}" submitted for review — a Keep/Discard card is in the room. It is not live yet.`
        : `Skill draft "${name}" submitted for review — a Keep/Discard card is in the room. It is not live yet.`,
    );
  },
};
...
/**
 * The authoring prompt for a learning review.
 *
