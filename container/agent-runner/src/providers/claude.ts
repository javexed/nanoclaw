      ...
      for await (const message of sdkResult) {
        ...
        if (message.type === 'system' && message.subtype === 'init') {
          ...
          if (Array.isArray(content)) {
            // web: thinking blocks ride the same content array. Widen locally
            // rather than edit upstream's cast above, so that line stays byte-identical.
            ...
            const thinking = (content as Array<{ type?: string; thinking?: string }>)
