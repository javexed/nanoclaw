...
export function writeCodexConfigToml(
  ...
  for (const [name, config] of Object.entries(servers)) {
    // Codex's config.toml only speaks stdio MCP servers (command/args/env). Remote
    // (sse/http) servers have no representation here, so skip them — a remote server
    // in the shared MCP config would otherwise break the Codex provider. Those tools
    // just aren't visible to Codex (the Claude provider still uses them).
    if (!('command' in config)) {
      log(`Codex: skipping MCP server '${name}' — remote (sse/http) servers aren't supported in config.toml`);
      continue;
    }
