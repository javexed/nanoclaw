        ...
        retryWith: async (provider, providerName) => {
          ...
          try {
            return await processQuery(
              ...
              prompt,
              undefined,
              // This is the escalation retry path — it merged cleanly and so
              // silently bound turnContext to the new emitsMidTurnText slot.
              // The retry runs the SAME provider, so it carries the same
              // capability as the call it is retrying.
  ...
  try {
    for await (const event of query.events) {
      ...
      if (event.type === 'init') {
        ...
        // hook and after the turn-boundary reset above, so an observer sees a
        // fully closed-out turn. Not awaited: the event drain must never stall.
