...
- **Built-in web UI** — a single-user browser PWA served by the host: rooms, a live thinking bubble, file attachments, approval cards, and a management drawer for agents and models (including a local-Ollama console). In-tree; enable at setup or with `/add-web`. See [docs/web/USAGE.md](docs/web/USAGE.md).
